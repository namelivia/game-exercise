...
class QuitGame(Command):
    def __init__(self, queue: "Queue"):
        super().__init__(queue, "Exit from the game")
        self.events = [QuitGameEvent()]


# ======= GAME STATE SYNC =======
class InitiateGame(Command):
    def __init__(self, queue: "Queue", game_data: "GameData"):
        super().__init__(queue, f"Locally initializing game {game_data.id}")
        self.events = [
...
class SetPlayerName(Command):
    def __init__(self, queue: "Queue", name: str):
        super().__init__(queue, "Setting player name")
        self.events = [
...
class GameCreatedInGameCommand(Command):
    def __init__(self, queue: "Queue", player_id: "UUID"):
        super().__init__(queue, f"Player {player_id} created a game")
...
class PlayerJoinedInGameCommand(Command):
    def __init__(self, queue: "Queue", player_id: "UUID"):
        super().__init__(queue, f"Player {player_id} joined the game")
        self.events = [
            PlayerJoinedInGameEvent(
                player_id
            )  # Event to be picked up by the screen event handler
...
class PlayerWinsInGameCommand(Command):
    def __init__(self, queue: "Queue", player_id: "UUID"):
        super().__init__(queue, f"Player {player_id} wins the game")
        self.events = [
            PlayerWinsInGameEvent(
                player_id
            )  # Event to be picked up by the screen event handler
...
class PingTheServer(Command):
    def __init__(self, queue: "Queue"):
        super().__init__(queue, "Ping the server")
